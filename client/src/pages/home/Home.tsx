...
import { useGetAllInvoiceQuery } from "../../feature/invoice/invoiceApiSlice.ts";
import DataTable, { TableColumn } from "react-data-table-component";
import { MdDeleteOutline } from "react-icons/md";
import { useGetAllUsersQuery } from "../../feature/user/userApiSlice.ts";
const Home = () => {
  // get data from state
  const { data, error, isLoading, refetch } = useGetAllInvoiceQuery("ok");
  const { data: userGetData } = useGetAllUsersQuery("ok");
  // type data
  ...
  const columns: TableColumn<UserData>[] = [
    ...
    {
      name: "Gender",
      selector: (row: UserData) => row.gender,
      sortable: true,
    },

    {
      ...
      selector: (row: UserData) => (
        <>
          <button className=" rounded bg-red-500 hover:bg-white hover:text-red-500  p-1 ">
  ...
  const { data: invoicesData } = useGetAllInvoiceQuery("ok");
  const { data: usersData } = useGetAllInvoiceQuery("ok");
  const { data: clientsData } = useGetAllInvoiceQuery("ok");
        ...
        <div className="relative flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 border border-blue-gray-100 shadow-sm">
          ...
          <div className="border-t border-blue-gray-50 p-4">
            <p className="block antialiased font-sans text-base leading-relaxed font-normal text-blue-gray-600">
              <strong className="text-green-500">{invoicesPercentage}%</strong>
              &nbsp;than last week
            </p>
          </div>
        </div>
